...
import type { BatchactionsJobRow } from '../types/PrismaClientInterface.js';
...
export function toRow(state: JobState): Omit<BatchactionsJobRow, never> {
  return {
    id: state.id,
    status: state.status,
    config: JSON.stringify(stripNonSerializableFields(state.config)),
    ...
    startedAt: state.startedAt !== undefined ? BigInt(state.startedAt) : null,
    completedAt: state.completedAt !== undefined ? BigInt(state.completedAt) : null,
    distributed: state.distributed ?? false,
  };
}

export function toDomain(row: BatchactionsJobRow): JobState {
