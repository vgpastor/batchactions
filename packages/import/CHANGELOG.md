...
- **`BulkImport.restore()` silent failure without stateStore** — Now throws a descriptive error when `stateStore` is not provided, instead of silently returning `null` (which conventionally means "job not found").
