...
import type { ProcessingContext } from '../../src/domain/ports/RecordProcessor.js';
...
describe('DuplicateChecker', () => {
  ...
  it('should pass ProcessingContext to checker', async () => {
    ...
    expect(capturedContext?.jobId).toBeTruthy();
    expect(capturedContext?.batchId).toBeTruthy();
    expect(capturedContext?.batchIndex).toBe(0);
    expect(capturedContext?.recordIndex).toBe(0);
  });
